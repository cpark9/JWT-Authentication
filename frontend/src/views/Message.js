...
function Message() {
  ...
  // React Router v6용 이동 훅
  const navigate = useNavigate()

  useEffect(() => {
    try {
      axios.get(baseURL + '/my-messages/').then((res) => {
              ...
              <div className="col-12 col-lg-5 col-xl-3 border-right">
                ...
                {messages.map((messages) => 
                  <a href="#" className="list-group-item list-group-item-action border-0">
                    ...
                    <small><div className="badge bg-success float-right text-white">{moment.utc(messages.date).local().startOf('seconds').fromNow()}</div></small>
                    <div className="d-flex align-items-start">
                        {messages.sender.id !== user_id &&
                          <img src={messages.sender_profile.image} className="rounded-circle mr-1" width={40} height={40}/>
                        }
                        {messages.sender.id === user_id &&
                          <img src={messages.reciever_profile.image} className="rounded-circle mr-1" width={40} height={40}/>
                        }
                      <div className="flex-grow-1 ml-3">
                        {messages.sender.id === user_id && 
                          (messages.reciever_profile.full_name !== null ? messages.reciever_profile.full_name : messages.reciever.username)
                        }
                        {messages.sender.id !== user_id && 
